...
class BWVNavigationMenu {
  ...
  async loadAvailableWorks() {
    try {
      const response = await fetch('/hyparts.json');

      if (!response.ok) {
        throw new Error(`Failed to load hyparts.json: ${response.status}`);
      }

      const hypartsData = await response.json();

      if (hypartsData.lagu && Array.isArray(hypartsData.lagu) && hypartsData.lagu.length > 0) {
        // // Numerical sorting by BWV number
        // this.availableWorks = hypartsData.lagu.sort((a, b) => {
        //   // Extract numeric part (remove 'bwv' prefix)
        //   const numA = parseInt(a.replace(/^bwv/, ''), 10);
        //   const numB = parseInt(b.replace(/^bwv/, ''), 10);
        //   return numA - numB;
        // });
        this.availableWorks = hypartsData.lagu.sort();
      } else {
        throw new Error('Invalid hyparts.json format or empty BWV list');
      }

      this.isLoaded = true;

    } catch (error) {
      console.warn('⚠️ Failed to load BWV list from hyparts.json:', error.message);

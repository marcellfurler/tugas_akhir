...
def process_fermata_csv(fermata_csv_path, notes_data):
    ...
    with open(fermata_csv_path, 'r', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            # Updated: expect data_ref column
            data_ref = row.get('data_ref', '').strip()
            ...
            if data_ref in data_ref_to_notes:
                ...
                print(
                    f"  No direct match for {data_ref}, using spatial interpolation...")
                ...
                if fermata_tick is not None:
                    fermata_items.append([fermata_tick, None, None, 'fermata'])
                    print(
                        f"  Added interpolated fermata at tick {fermata_tick} for {data_ref} (x={fermata_x})")
                else:
                    print(
                        f"  Warning: Could not interpolate fermata position for {data_ref}")
...
def consolidate_fermatas_by_measure(fermata_items, bars):
    ...
    if len(unique_fermatas) != len(fermata_items):
        print(
            f"  Removed {len(fermata_items) - len(unique_fermatas)} exact duplicates")
    ...
    for fermata in unique_fermatas:
        ...
        if not bar_ticks:
            ...
            for i in range(len(bar_ticks)):
                ...
                bar_end = bar_ticks[i + 1] if i + \
                    1 < len(bar_ticks) else float('inf')
    ...
    for measure_index, fermatas_in_measure in measure_fermatas.items():
        measure_name = "pickup" if measure_index == - \
            1 else f"measure {measure_index}"

        if len(fermatas_in_measure) == 1:
            # Single fermata - keep as is
            consolidated_fermatas.append(fermatas_in_measure[0])
            print(
                f"  {measure_name}: kept single fermata at tick {fermatas_in_measure[0][0]}")
        else:
            ...
            removed_ticks = [f[0]
                             for f in fermatas_in_measure if f != latest_fermata]
            print(
                f"  {measure_name}: consolidated {len(fermatas_in_measure)} fermatas")
            print(f"    Kept fermata at tick {latest_fermata[0]} (rightmost)")
            print(f"    Removed fermatas at ticks {removed_ticks}")

    print(
        f"📍 Consolidated to {len(consolidated_fermatas)} fermatas ({len(unique_fermatas) - len(consolidated_fermatas)} removed by measure)")
...
def interpolate_fermata_by_position(fermata_x, before_note, after_note):
    ...
    try:
        if before_note is None and after_note is None:
            ...
            fermata_tick = int(
                before_note['off_tick'] + (relative_pos * tick_span))

            print(
                f"    Interpolated: x={fermata_x} between {before_note['x']} and {after_note['x']} -> tick {fermata_tick}")
            return fermata_tick
...
def calculate_channel_stats(notes_data):
    ...
    for note in notes_data:
        ...
        # Default to middle C (MIDI 60) if missing
    ...
    for channel in sorted(channel_stats.keys()):
        ...
        print(
            f"  Channel {channel}: {stats['minPitch']}-{stats['maxPitch']} (avg: {avg_pitch:.1f}), {stats['count']} notes")
...
def extract_bars_from_svg(svg_root, meta, config_data):
    ...
    last_measure_duration_str = config_data.get(
        'musicalStructure', {}).get('lastMeasureDuration')
    if last_measure_duration_str:
        musical_end_moment = moments[-1] + \
            parse_fraction(last_measure_duration_str)
    else:
...
def clean_svg(svg_root):
    ...
    for rect in svg_root.findall('.//*[@data-bar]'):
        ...
        essential_attrs = ['x', 'y', 'width',
                           'height', 'ry', 'transform', 'data-bar']
        attrs_to_remove = []
    ...
    for a_elem in elements_to_process:
        ...
        # Solmisasi-lily accidental line processing
        if len(accidental_line_elements) == 1:
            line_elem = accidental_line_elements[0]
            print(
                f"Processing accidental line: {data_ref} -> {simplified_ref}")
            new_line = ET.Element('line')
            for attr, value in line_elem.attrib.items():
                # Skip fill attribute (will be handled by CSS)
                if attr != 'fill':
        ...
        total_children = len(path_elements) + \
            len(text_elements) + len(accidental_line_elements) + \
            len(other_elements)
        ...
        if len(path_elements) == 1 and total_children == 1:
            ...
            # Multiple elements or text elements: convert <a> to <g> (group)
            # and preserve all children
            element_types = []
            if path_elements:
                element_types.append(f"{len(path_elements)} path(s)")
            if text_elements:
                element_types.append(f"{len(text_elements)} text element(s)")
            if accidental_line_elements:
                element_types.append(
                    f"{len(accidental_line_elements)} accidental line(s)")
            if other_elements:
                element_types.append(f"{len(other_elements)} other element(s)")

            print(
                f"Processing multiple elements: {data_ref} -> {simplified_ref} ({', '.join(element_types)})")
            ...
            print(
                f"Warning: Empty <a> element with data-ref {data_ref} - preserving as group")
    ...
    print(
        f"Moving {len(note_heads)} note heads to end of their parents for proper z-order")
    ...
    print(
        f"Successfully reordered note heads in {len(parents_to_update)} parent containers")
...
def generate_sync_files(svg_input, notes_input, config_input, svg_output, notes_output, fermata_input=None):
    ...
    for note in notes_data:
        ...
        flow_items.append(
            [note['on_tick'], channel, note['off_tick'], clean_data_refs])

        # Debug: print first few notes to verify channel data integrity
        if len(flow_items) <= 3:
            print(
                f"  Note: ticks {note['on_tick']}-{note['off_tick']}, channel {channel}, data_refs {clean_data_refs}")
    ...
    with open(notes_output, 'w') as f:
        ...
        for item in flow_items:
            ...
            # Bar: [tick, ~, bar_number, bar]
            if len(item) == 4 and item[3] == 'bar':
                f.write(f"- [{item[0]}, ~, {item[2]}, {item[3]}]\n")
            # Fermata: [tick, ~, ~, fermata]
            elif len(item) == 4 and item[3] == 'fermata':
                f.write(f"- [{item[0]}, ~, ~, {item[3]}]\n")
            # Note: [start_tick, channel, end_tick, hrefs]
            elif len(item) == 4:
    ...
    total_notes = len([item for item in flow_items if len(
        item) == 4 and item[3] != 'bar' and item[3] != 'fermata'])
    total_bars = len(
        [item for item in flow_items if len(item) == 4 and item[3] == 'bar'])
    total_fermatas = len(
        [item for item in flow_items if len(item) == 4 and item[3] == 'fermata'])

    print(
        f"✅ Generated {len(flow_items)} flow items ({total_notes} notes, {total_bars} bars, {total_fermatas} fermatas)")

    # Debug: Show channel distribution to verify multi-track processing
    channel_counts = {}
    for item in flow_items:
        # Note: [start_tick, channel, end_tick, hrefs]
        if len(item) == 4 and item[3] != 'bar' and item[3] != 'fermata':
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate unified sync format with optional fermata support (normalized pipeline)')

...
def remove_unwanted_hrefs(input_path: Path, output_path: Path):
    ...
    print(
        f"   ✅ Converted {xlink_conversion_count} xlink:href attributes to href")
    ...
    parent_map = {child: parent for parent in svg_root.iter()
                  for child in parent}
    ...
    for anchor_element in svg_root.findall(".//svg:a", NAMESPACE_MAP):
        ...
        contains_text = anchor_element.find(
            ".//svg:text", NAMESPACE_MAP) is not None
        contains_rect = anchor_element.find(
            ".//svg:rect", NAMESPACE_MAP) is not None
        contains_path = anchor_element.find(
            ".//svg:path", NAMESPACE_MAP) is not None

        # Check solmisasi
        is_solmisasi = parent_map.get(
            anchor_element).get("class") == "notangka"
        ...
        is_pure_annotation = (contains_text or contains_rect) and \
            ...
            not is_solmisasi
        should_remove = (
            matches_unwanted_pattern and not has_tie_attributes) or is_pure_annotation
    ...
    try:
        ...
        print(
            f"   🔧 Converted {xlink_conversion_count} legacy xlink:href to modern href")
        print(f"   🧹 Cleaned {href_cleaned_count} href references")
        ...
        print(
            f"   📏 File size change: {original_size:,} → {cleaned_size:,} bytes ({size_change:+,})")

        # Provide guidance on what was preserved
        preserved_links = total_anchor_count - removed_link_count
        if preserved_links > 0:
            print(
                f"   🎵 Preserved {preserved_links} musical noteheads as clean data-ref attributes")
